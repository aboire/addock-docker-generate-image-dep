...
docker build --pull --rm -t $REPO_PREFIX/addok-standalone-$DEPARTEMENT:$VERSION .
docker tag $REPO_PREFIX/addok-standalone-$DEPARTEMENT:$VERSION $REPO_PREFIX/addok-standalone-$DEPARTEMENT:$VERSION
[ ! -z "$PUSH_IMAGE" ] && docker push $REPO_PREFIX/addok-standalone-$DEPARTEMENT:$VERSION

# Tag supplémentaire pour la version "latest"
docker tag $REPO_PREFIX/addok-standalone-$DEPARTEMENT:$VERSION $REPO_PREFIX/addok-standalone-$DEPARTEMENT:$TAG
[ ! -z "$PUSH_IMAGE" ] && docker push $REPO_PREFIX/addok-standalone-$DEPARTEMENT:$TAG
